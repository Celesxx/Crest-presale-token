...
class LoadingHelper
{
    ...
    **/
    async loadAllContractFunction(address, provider, props)
    {
      ...
      props.dashboardAction({loading : {}, action: "loading"})
      data.maxUserToken = await contractHelper.getMaxUserToken(provider, 18)
      props.dashboardAction({loading : {}, action: "loading"})
      data.maxToken = await contractHelper.getMaxToken(provider, 18)
      props.dashboardAction({loading : {}, action: "loading"})
      data.isWhitelist = await contractHelper.getIsWhitelist(provider)
      props.dashboardAction({loading : {}, action: "loading"})
      data.remainingToken = await contractHelper.getRemainingToken(provider, 18)
      props.dashboardAction({loading : {}, action: "loading"})
      data.crestBuy = await contractHelper.getUserBuyToken(provider, address, 18)
      props.dashboardAction({loading : {}, action: "loading"})
      data.stableBalance = await contractHelper.getBalanceOf(provider, Address.stable, address, 18)
      props.dashboardAction({loading : {}, action: "loading"})
      data.crestBalance = await contractHelper.getBalanceOf(provider, Address.token, address, 18)
      props.dashboardAction({loading : {}, action: "loading"})
